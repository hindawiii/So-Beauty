...
export function SiteHeader() {
  ...
  const nav = [
    ...
    { to: "/offers", label: "العروض الحصرية", highlight: false },
    ...
    <header className="sticky top-0 z-40 bg-background/95 backdrop-blur-md border-b border-border/70 shadow-2xs">
      ...
      <div className="container mx-auto px-2.5 sm:px-6 h-15 sm:h-16 md:h-20 flex items-center justify-between gap-2 sm:gap-4 md:gap-6">
        ...
        <div className="hidden md:flex flex-1 max-w-md mx-2 lg:mx-6">
          ...
          >
            <span className="flex items-center gap-2.5">
              ...
              <span>ابحثي عن منتج، ماركة، أو مكون...</span>
            </span>
        ...
        <div className="flex items-center gap-1.5 sm:gap-2 md:gap-2.5 shrink-0">
          ...
          {/* 3. Smart Currency Switcher: Available on Mobile, Tablet & Desktop */}
          <div className="flex items-center shrink-0">
            <CurrencySwitcher />
          </div>

          {/* 4. Wishlist Link (Tablet and PC: >= sm) */}
          <Link
            to="/wishlist"
            className="relative p-1.5 sm:p-2 md:p-2.5 rounded-full text-slate-700 dark:text-slate-200 hover:text-rose-600 hover:bg-muted/70 transition-colors hidden sm:inline-flex cursor-pointer shrink-0"
            aria-label="قائمة المفضلة"
            title="المنتجات المحفوظة في المفضلة"
          >
          ...
          {/* 5. Shopping Cart Button (Always visible on Mobile, Tablet & PC) */}
          <Link
            to="/cart"
